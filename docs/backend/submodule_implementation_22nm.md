...
- `v_th`：选择标准单元库的阈值电压。
- `proj(analysis_view,[setup|hold|power])`：选择时序分析的 PVT，学术片考虑 `tt` 的两个选项即可。
- `top_module`：顶层模块的名称。
- `clock_period`：时钟周期，单位 ns。
...
- `metal_option`：金属结构选择，目前只支持 `1p9m_6x1z1u`。
- `metal_direction`：M2 金属绕线方向，推荐 `horizontal`。
- `init_pwr_net`：电源端口名称。
- `init_gnd_net`：接地端口名称。
...
<figure>
  <img src="../assets/images/shared/innovus_startup.png" width=80%>
...
- `<top_module_name>_flat_postpnr.v.gz`：物理实现后带有电源的 flatten 网表，用于 **LVS 检查**。
- `<top_module_name>_hier_postpnr.v.gz`：物理实现后没有电源的层次化网表，用于**后仿**。
- `<top_module_name>_tt0p8v25c.lib`：物理实现后的时序信息库，用于**顶层模块集成**。
- `<top_module_name>_tt0p8v25c.sdf.gz`：物理实现后的标准延迟表，用于**后仿**。
- `<top_module_name>.lef`：物理实现后的版图信息，用于**顶层模块集成**。
- `<top_module_name>.cdl.gz`；物理实现后的电路描述文件，用于 **LVS 检查**。
- `<top_module_name>.gds.gz`：物理实现后的**版图文件**。
...
```shell
make restore_innovus STAGE=<stage_name>
```
...
<figure>
  <img src="../assets/images/shared/timing_closure_flow.png" width=80%>
...
!!! Warning "关于图片的说明"
    物理设计流程中的所有图片均为**示意图**，与模板中实际运行出来的结果会**有出入**！
...
<figure>
  <img src="../assets/images/shared/check_floorplan_view.png" width=100%>
...
<figure>
  <img src="../assets/images/shared/init_floorplan_view.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/floorplan_box.png" width=70%>
...
<figure>
  <img src="../assets/images/shared/example_floorplan.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/macro_move.png" width=100%>
  <figcaption>Move/Resize/Reshape Widget</figcaption>
</figure>

<figure>
  <img src="../assets/images/shared/move_orthogonal.png" width=100%>
...
<figure>
  <img src="../assets/images/shared/floorplan_toolbox.png" width=100%>
...
<figure>
  <img src="../assets/images/shared/place_macro.png" width=80%>
...
``` tcl
addHaloToBlock [list 4 4 4 4] -allMacro
```
...
#### 添加 Pin（`pnr/scripts/floorplan/signal_pin.tcl`）
...
<figure>
  <img src="../assets/images/shared/pin_editor.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/add_pin.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/add_core_rings.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/add_block_rings.png" width=80%>
...
* `-stacked_via_bottom_layer`：指定 stripe 能通过通孔连接到的最底层金属层。对于**最底层**的 Stripe，该值设定为 **M1**；对于**其他**的 stripe，该值一般设定为**更低的一层**。
* `-stacked_via_top_layer`：指定 stripe 能通过通孔连接到的最高层金属层。一般设定为当前层。
* `-nets`：指定 Power Stripe 的信号名称，同时这些电源/地信号会被认为是**一个组（set）**。
* `-layer`：指定 Power Stripe 的金属层。
* `-direction`：指定 Power Stripe 的方向，可以选择 `horizontal` 或 `vertical`。
* `-width`：指定 Power Stripe 的宽度。
* `-spacing`：指定一组 Power Stripe 之间的间距，即相邻电源线最相近的两条边之间的距离。
* `-start_offset`：指定 Power Stripe 距离版图边界的距离。
* `-set_to_set_distance`：指定组与组之间的间距，相邻两组对应位置之间的距离。
* `-block_ring_bottom_layer_limit`：指定 Power Stripe 在遇到 Block ring 时可以切换到的最底层金属层。
* `-block_ring_top_layer_limit`：指定 Power Stripe 在遇到 Block ring 时可以切换到的最高层金属层。
* `-padcore_ring_bottom_layer_limit`：指定 Power Stripe 在遇到 Pad/Core ring 时可以切换到的最底层金属层。
* `-padcore_ring_top_layer_limit`：指定 Power Stripe 在遇到 Pad/Core ring 时可以切换到的最高层金属层。
...
<figure>
  <img src="../assets/images/shared/sram_pg_connection.png" width=90%>
...
<figure>
  <img src="../assets/images/shared/add_power_stripes.png" width=80%>
...
* `-layerChangeRange`：指定可以改变的金属层范围。
* `-crossoverViaLayerRange`：指定可以穿越的金属层范围。
* `-targetViaLayerRange`：指定目标金属层范围。
* `-allowJogging`：是否允许走线时的弯曲。
* `-allowLayerChange`：是否允许金属层的改变。
...
!!! danger "`sroute` 之后 GUI 中 Macro 周围的 Violation"
    ...
    <figure>
      <img src="../assets/images/shared/power_rail_open.png" width=80%>
    ...
    <figure>
      <img src="../assets/images/shared/power_rail_open_zoomin.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/sroute.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/place_blockage.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/add_endcap_cells.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/add_welltap_cells.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/add_decap_cells.png" width=80%>
...
``` tcl
deleteTieHiLo
setPlaceMode -place_detail_preroute_as_obs {7}
place_opt_design
addTieHiLo
```

* `deleteTieHiLo`：从门级网表中删除目前摆放的 Tie-high 和 Tie-low 标准单元，将相应信号重新连接到逻辑高电平和逻辑低电平。此处可以理解为初始化操作。
* `setPlaceMode -place_detail_preroute_as_obs {7}`：限制标准单元不能摆放在已有的 M7 金属线之下。
...
<figure>
  <img src="../assets/images/shared/place_layout.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/clk_tree_debug.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/ctd.png" width=80%>
...
!!! danger "时序收敛"
    请**确保**此处的 setup 和 hold **不要违例**。如果违例，请确保违例保证在 **5ps 以内**。
...
* `editDelete -regular_wire_with_drc`：删除所有带有 DRC 违例的常规布线。
...
<figure>
  <img src="../assets/images/shared/drc_fix_pre.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/drc_fix_post.png" width=80%>
...
#### 版图填充（`pnr/scripts/finish/filler_decap.tcl`）
...
#### 添加电源端口（`pnr/scripts/finish/pin_label.tcl`）
...
<figure>
  <img src="../assets/images/shared/add_pgpin.png" width=80%>
...
<figure>
  <img src="../assets/images/shared/pgpin_added.png" width=80%>
  <figcaption>Power/Ground Pin</figcaption>
</figure>

#### 结果文件导出（`pnr/scripts/finish/file_gen.tcl`）

**直接运行**`pnr/scripts/finish/file_gen.tcl` 脚本即可导出 Innovus 的结果文件。
该脚本会在 `pnr/<top_module_name>` 文件夹下生成 Innovus 的结果文件，包括如下几个文件：

- `<top_module_name>_flat_postpnr.v.gz`：物理实现后带有电源（VDD, VSS 等）的 Flatten 网表，用于 **LVS 检查**。
- `<top_module_name>_hier_postpnr.v.gz`：物理实现后没有电源的层次化网表，用于**后仿**。
- `<top_module_name>_tt0p8v25c.lib`：物理实现后的时序信息库，用于**顶层模块集成**。
- `<top_module_name>_tt0p8v25c.sdf.gz`：物理实现后的标准延迟表，用于**后仿**。
- `<top_module_name>.lef`：物理实现后的版图信息，用于**顶层模块集成**。
- `ilm/`：interface logic model，用于顶层模块集成时**提供子模块接口的时序信息**。
- `<top_module_name>.gds.gz`：物理实现后的**版图文件**。
- `<top_module_name>.cdl.gz`：物理实现后的**CDL（Circuit Description Language）网表文件**，用于 LVS 检查。
