...
- `soc_axi/reg_bus/reg_intf.sv`：寄存器总线接口定义。
- `soc_axi/axi_bus/include/axi_intf.sv`：AXI 总线接口定义。
- `soc_axi/bus_converter/axi_to_reg.sv`：AXI 到寄存器总线转换器。

如果 AXI 总线的数据位宽和寄存器总线的数据位宽不一致，你还需要调用：

- `soc_axi/bus_converter/axi_dw_converter.sv`：AXI 数据位宽转换器。

接下来，你需要理解**寄存器总线接口**、阅读 `soc_axi/include/reg_*.svh` 并**自行编写**寄存器与寄存器总线之间的逻辑，如下是一个简单的例子。
...
- `soc_axi/axi_bus/include/axi_intf.sv`：AXI 总线接口定义。
- `soc_axi/bus_converter/axi2mem.sv`：AXI 到 memory 接口转换器。
...
- `soc_axi/soc_pkg.sv`：地址映射定义。
- `soc_axi/soc.sv`：SoC 顶层模块。
