...
??? info "22nm vs. 28nm"
    28nm和22nm属于同一个工艺节点，这意味着二者的区别**仅限于尺寸**，其他的例如 DRC、LVS 规则**完全相同**。EDA 工具中，无论是22nm还是28nm工艺，其坐标或者其他尺寸均为28nm下的尺寸。22nm工艺只会在流片的时候进行尺寸缩放（$\times0.855$）。
...
图（a）∼（h）展示了EDA工具的流程和中间结果的简单描述（注：为了便与显示，e∼g
中Powerplan 的结果被暂时隐藏）。
