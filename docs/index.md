...
!!! info "22nm vs. 28nm vs. 32nm"
    22nm、28nm 和 32nm 属于同一个工艺节点，这意味着三者的区别**仅限于尺寸**，其他的例如 DRC、LVS 规则**完全相同**。EDA 工具中，无论是 22nm、28nm 还是 32nm 工艺，其坐标或者其他尺寸均为 32nm 下的尺寸。22nm 工艺只会在流片的时候进行尺寸缩放（长宽均 $\times0.855$）。
...
图 (a)-(h) 展示了 EDA 工具的流程和中间结果的简单描述（注：为了便于显示，(e)-(g) 中 Powerplan 的结果被暂时隐藏）。
在该流程当中，综合阶段的步骤 (a)-(b) 较少且**自动化程度较高**，用户只需要设置好输入的文件和参数即可运行工具得到综合结果。
后端实现部分 (b)-(h) 的步骤较多，为追求芯片极致的性能，工程师们往往需要分析各个阶段的结果
并且必要时进行**手动设计**，例如手动实现布局规划和手动解决设计规则约束（Design Rule Constraint, DRC）违例的问题。
...
* [I. EDA 工具与环境背景](background/eda_tools_environment.md)
* II. 前端设计
    * RTL 设计
        * [SRAM IP 使用规范](frontend/rtl_design/sram_ip_spec.md)
        * [FPU IP 使用规范](frontend/rtl_design/fpu_ip_spec.md)
        * [系统集成](frontend/rtl_design/system_integration.md)
    * 数字子系统逻辑综合
        * [TSMC N22](frontend/submodule_synthesis_22nm.md)
        * [TSMC N12](frontend/submodule_synthesis_12nm.md)
* III. 后端实现
    * 数字子系统后端设计
        * [TSMC N22](backend/submodule_implementation_22nm.md)
        * [TSMC N12](backend/submodule_implementation_12nm.md)
    * 顶层模块后端设计
        * [TSMC N22](backend/top_io_implementation_22nm.md)
        * [TSMC N12](backend/top_io_implementation_12nm.md)
    * 物理验证
        * [模块物理验证](backend/lvs_drc_block.md)
        * [顶层物理验证](backend/lvs_drc_top.md)
    * [拼版](backend/final_layout.md)
* IV. 验证
    * [数字子系统仿真](verification/simulation.md)
    * [FPGA 验证](verification/fpga_verification.md)
    * [静态时序分析与功耗分析](verification/sta_power.md)
* V. 回片测试
    * [PCB 绘制](post_tapeout/pcb.md)
    * [封装](post_tapeout/package.md)
    * 测试
        * [测试仪器使用指南](post_tapeout/testing/testing_equipment.md)
        * [RISC-V CPU 调试](post_tapeout/testing/cpu_debugging.md)
* VI. 附录
    * [C 代码编译](appendix/c_compiling.md)
    * [WSL](appendix/wsl.md)
    * [PDK 设置](appendix/pdk_setup.md)
    * [通过 FPGA 使用 DRAM](appendix/fpga_dram.md)
    * [Linux 环境与常用工具](appendix/linux_environment.md)
* VII. 旧版文档
    * [Digital-CIM 使用规范（Deprecated）](legacy/dcim_ip_spec.md)
    * [行为级仿真（Deprecated）](legacy/behavioral_simulation.md)
    * [门级仿真（Deprecated）](legacy/gate_level_simulation.md)
    * [子模块综合（Deprecated）](legacy/submodule_synthesis_deprecated.md)
    * [子模块综合（New）](legacy/submodule_synthesis_new.md)
    * [子模块后端（Deprecated）](legacy/submodule_implementation_deprecated.md)
    * [子模块后端（New）](legacy/submodule_implementation_new.md)
* [VIII. 如何贡献](contribute.md)
