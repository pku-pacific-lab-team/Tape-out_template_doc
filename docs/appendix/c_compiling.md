...
```C
uint64_t *ptr = (uint64_t *)0x80000000; // 指定地址
*ptr = 0x123456789ABCDEF0; 				// 将数据写入指定地址
```

上述方法会导致数据**被存储了两份**：一份在编译出来的目标文件中，另一份在运行时被写入到指定地址。
如果我们只想存储一份数据，可以定义一个静态数组，并在编译时通过**链接器脚本** `linker.ld` 将其放置到指定地址。
